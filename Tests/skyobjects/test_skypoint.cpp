...
void TestSkyPoint::compareNovas()
{
    ...
    qDebug() << "KDT JD " << static_cast<double>(jd) << "  LN JD " << lnJd << " Diff " << (static_cast<double>(jd) - lnJd) * 86400 << "secs";
...
void TestSkyPoint::testPrecess()
{
    ...
    qDebug() << "precess dRa " << sp.ra0().Degrees() - sp.ra().Degrees() << ", dDec " << sp.dec0().Degrees() - sp.dec().Degrees();
...
void TestSkyPoint::testNutate()
{
    ...
    qDebug() << "nutate dRa " << sp.ra0().Degrees() - sp.ra().Degrees() << ", dDec " << sp.dec0().Degrees() - sp.dec().Degrees();
...
void TestSkyPoint::compare(QString msg, double ra1, double dec1, double ra2, double dec2, double err)
{
    ...
    QVERIFY2(fabs(ra1 - ra2) < err, qPrintable(QString("Ra %1, %2 error %3").arg(ra1).arg(ra2).arg(((ra1-ra2) * 3600.0),6,'f',1)));
    QVERIFY2(fabs(dec1 - dec2) < err, qPrintable(QString("Dec %1, %2 error %3").arg(dec1).arg(dec2).arg((dec1-dec2) * 3600.,6,'f',1)));
}
